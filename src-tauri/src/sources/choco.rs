...
/// Chocolatey backend. Uses `-r` (`--limit-output`) everywhere, which gives
/// clean pipe-delimited rows with no banners or headers.
pub struct Choco;
...
impl PackageSource for Choco {
    ...
    async fn list_installed(&self) -> anyhow::Result<Vec<Package>> {
        // Chocolatey v2+: `list` is local-only by default.
    ...
    async fn list_updates(&self) -> anyhow::Result<Vec<Package>> {
        // Rows: name|currentVersion|availableVersion|pinned
