...
/// Scoop backend. Scoop is a PowerShell function, so everything is invoked
/// through `powershell`. Installed apps come from `scoop export` (JSON);
/// search and updates parse Scoop's whitespace tables.
pub struct Scoop;
...
/// Split a Scoop output table into data rows. Finds the `----` separator line
/// and returns each following non-empty row split into whitespace columns.
    ...
    fn parse_search(out: &str) -> Vec<Package> {
        table_rows(out)
            ...
            .filter_map(|cols| {
                ...
                pkg.publisher = cols.get(2).cloned(); // bucket
                Some(pkg)
            })
            .collect()
    }
...
impl PackageSource for Scoop {
    ...
    async fn list_updates(&self) -> anyhow::Result<Vec<Package>> {
        // Columns: Name, Installed Version, Latest Version, ...
        ...
        let pkgs = table_rows(&out)
            ...
            .filter_map(|cols| {
                ...
                // A real update row has a distinct latest version.
