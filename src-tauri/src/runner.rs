...
/// Stop spawned console programs from flashing a window on Windows.
/// `creation_flags` lives on the std command, so configure there then convert.
#[cfg(windows)]
...
/// Spawn a command and stream its stdout/stderr to the frontend line by line as
/// `event` events. Resolves with the process exit code.
pub async fn stream(
    app: &AppHandle,
    op_id: &str,
    event: &str,
    ...
    // Drain stderr concurrently so it can't block the pipe.
    let err_task = {
        let app = app.clone();
...
/// Wrap a PowerShell snippet so its stdout is UTF-8 and free of progress noise.
