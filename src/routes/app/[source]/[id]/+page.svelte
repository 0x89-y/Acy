<script lang="ts">
  ...
  // Every manager this app can be installed from. For a curated entry that's its
  // primary source plus alternates (enabled only); otherwise just the URL source.
  ...
  // Installed / update status from the shared cache (no fresh fetch). An app may
  // be installed via any of its variants, so match across all of them.
  ...
  // Add a searched (non-curated) app to the user's list. Once saved the store
  // refresh makes `curatedApp` non-null, so the button becomes "Edit".
...
<style>
  ...
  /* Left column = the app's identity + actions (the rail slot everywhere else). */
  .side {
    flex: 0 0 260px;
    display: flex;
  ...
  /* Facts are a flush divided list, edge-to-edge in the pane. */
  .facts {
    display: flex;
    flex-direction: column;
  }
