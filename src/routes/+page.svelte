<script lang="ts">
  ...
  // Back target for a search result's detail page, so "Back" returns to this
  // search rather than the Discover home. Restored from ?q= on mount below.
  ...
  // Attention banner for sources that need setup - more visible than the rail
  // dot alone. Dismissible for the session; reappears on reload / new issues.
  ...
  onMount(() => {
    ...
    // Restore a search when returning from a result's detail page (?q=...), so
    // Back lands on the search we were on rather than the Discover home.
...
<style>
  ...
  /* Attention banner: shown above the panel when a source needs setup, so it
     isn't missed the way the rail dot can be. */
  .setup-banner {
    flex-shrink: 0;
    display: flex;
    align-items: center;
  ...
  /* Attention dot on Library: a source needs setup/attention (updates are
     surfaced by the Updates entry's count badge in both rails instead). */
  .lib-dot {
    width: 6px;
    height: 6px;
    border-radius: 50%;
